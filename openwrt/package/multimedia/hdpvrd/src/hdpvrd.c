...
int main(int argv, char **argc)
{
	...
	openlog("hdpvrd", LOG_PID|LOG_CONS, LOG_DAEMON);
	...
	*p_int = 1;
	...
	if (bind(hsock, (struct sockaddr *)&my_addr, sizeof(my_addr)) == -1) {
		perror("Error binding to socket, make sure nothing else is listening on this port");
	...
	while (1) {
		     ...
		     accept(hsock, (struct sockaddr *)&sadr, &addr_size)) != -1)
		{
			...
			pthread_create(&thread_id, 0, &SocketHandler,
				       (void *)datainst);
			pthread_detach(thread_id);
		} else {
...
void *BufferCleanUp(void *lp)
{
	...
	while (1) {
		...
		global->historyFree[global->historyNowIndex] =
		    global->freecount;
		global->historyUsed[global->historyNowIndex] =
		    global->buffercount;
		...
		if ((freecounts[0] >> 10)
		    ...
		    && (freecounts[4] >> 10)
		    && (freecounts[5] >> 10)
		    && (freecounts[6] >> 10) && (freecounts[7] >> 10)) {
			...
			for (freeloop = 0; freeloop < 512; freeloop++) {
				char *buf =
				    global->freebuffers[global->freecount - 1];
				free(buf);
				global->freecount--;
			}
		}
	...
	for (outfdcount = 0; outfdcount < currentmax; outfdcount++) {
		if (global->outfds[outfdcount] != -1) {
			if (writepos != outfdcount) {
				...
				global->outfds[writepos] =
				    global->outfds[outfdcount];
...
void sendBuffersToFds(int buffersToWrite, struct shared *global)
{
	...
	for (outfdcount = 0; outfdcount < (global->recording); outfdcount++) {
		if (global->outfds[outfdcount] != -1) {
			for (buffercount = 0; buffercount < buffersToWrite;
			     buffercount++) {
				ssize_t written =
				    write(global->outfds[outfdcount],
					  global->data[buffercount],
					  MEMBUF_SIZE);
				if (written == -1) {
					global->outfds[outfdcount] = -1;
				}
	...
	while (1) {
		...
		while (enabled || bufferstowrite > 0) {
			...
			if (bufferstowrite == 0) {
				...
				memcpy(&
				       (global->freebuffers[global->freecount]),
				       global->data,
				       (bufferstowrite * sizeof(char *)));
...
void *StreamHandler(void *lp)
{
	...
	if (-1 == (devfd = open(ifname, O_RDWR | O_NONBLOCK))) {
		perror("Unable to open hdpvr device");
	...
	while (1) {
		...
		while (countdown > 0) {
			...
			if (0 == retval) {
				...
				// Last resort
				if (countdown == 0) {
					perror("Given up, rebooting the system");
					syslog(LOG_CRIT, "Given up, rebooting the system");
					system("/sbin/reboot");
				}
				time(&timer);
				tm_info = localtime(&timer);
				strftime(buffer, 80, "%Y-%m-%d %H:%M:%S",
					 tm_info);
				printf("Waiting for ready (%d)...", countdown);
				syslog(LOG_WARNING, "Waiting for ready (%d)...", countdown);
				usleep(500);
			} else {
				countdown = 0;
			}
		...
		if (0 == retval) {
			...
			printf("Lost signal, restarting device...\n");
			syslog(LOG_WARNING, "Lost signal, restarting device...");
			close(devfd);
			countdown = 5;
			if (-1 == (devfd = open(ifname, O_RDWR | O_NONBLOCK))) {
				perror("Unable to reopen hdpvr device");
				syslog(LOG_CRIT, "Unable to reopen hdpvr device");
				exit(EXIT_FAILURE);
			} else {
				fds[0].fd = devfd;
				fds[0].events = POLLIN;
				printf("Device reopened. Waiting for data\n");
				syslog(LOG_INFO, "Device reopened. Waiting for data");
				usleep(500);
				continue;
			}
		} else if (-1 == retval) {
			perror("Polling failed");
			syslog(LOG_CRIT, "polling failed");
			exit(EXIT_FAILURE);
			...
			if (0 > nbytes) {
				...
				default:
					perror("Unknown response when reading device");
					syslog(LOG_CRIT, "Unknown errno response %d when reading device", errno);
					exit(EXIT_FAILURE);
				}
				...
				if (recording) {
					...
					if (!
						...
						syslog(LOG_INFO, "Out of write buffers!! - reusing last buffer.. ");
						bufToUse =
						    global->
						    data[(global->
							  buffercount)-1];
					...
					if (bufToUse == NULL
					    && (global->buffercount +
						global->freecount)<MAX_BUFFERS)
					{
						if (NULL ==
						    (bufToUse =
						     malloc(MEMBUF_SIZE))) {
							printf("Not enough memory to allocate buffer (%d)\n", (global->freecount + global->buffercount));
							syslog(LOG_CRIT, "Not enough memory to allocate buffer (%d)", (global->freecount + global->buffercount));
							exit(EXIT_FAILURE);
						}
						global->data[global->
							     buffercount] =
...
/*
// Why is this bad? Let it loop & try again?
			 else {
				perror("Short read");
				syslog(LOG_CRIT, "Short read");
				exit(EXIT_FAILURE);
			}
*/
		} else if (fds[0].revents & POLLERR) {
			perror("pollerr");
			syslog(LOG_CRIT, "Poll error");
			exit(EXIT_FAILURE);
			break;
		}
	}
